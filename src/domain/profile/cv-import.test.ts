...
`;
  ...
  it('extracts useful deterministic profile candidates', () => {
  ...
  it('applies only explicitly selected values', () => {
    const profile = createEmptyStoredProfile();
    const draft = parseCvText(text);
    let nextId = 0;
    ...
    expect(next.baseProfile.professional.skills).toHaveLength(4);
